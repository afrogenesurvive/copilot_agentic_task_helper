...
4. Pending tool calls: read from `logs/pending-tool-calls/queue.jsonl` (that's the authoritative queue)
5. Tool call logs: use `/tool-logs` endpoint on the webhook server or read `logs/tool_call/YYYY-MM-DD.log` directly
